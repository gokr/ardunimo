...
const doNotUnmap = not (defined(amd64) or defined(i386)) or
                   defined(windows) or defined(nimAllocNoUnmap) or defined(nimArmEmbedded)
...
elif defined(nimArmEmbedded):
  proc osAllocPages(size: int): pointer {.inline.} =
    result = cmalloc(size)
    if (cast[int](result) and (PageSize-1)) != 0:
      cfree(result)
      result = cmalloc(size+PageSize)
      result = cast[pointer](cast[int](result) +% PageSize -% (cast[int](result) and (PageSize-1))) # align the pointer somehow
